...
function initialize_affiliate_scripts() {
	if ( is_admin() && isset( $_GET[ 'page' ] ) && $_GET[ 'page' ] == 'extensions.php' ) {
		...
		WP_Installer_Setup( $wp_installer_instance, array(
			...
			'src_name'				 => 'eCommerce Product Catalog', // optional, default value: empty, needed for coupons
...
function ic_show_affiliate_content() {
	...
	$config[ 'box_title' ]		 = 'Multilingual eCommerce Product Catalog';
	$config[ 'name' ]			 = 'eCommerce Product Catalog'; //name of theme/plugin
	$config[ 'box_description' ] = 'eCommerce Product Catalog is fully compatible with WPML - the WordPress Multilingual plugin. WPML lets you add languages to your existing sites and includes advanced translation management.';
	...
	if ( $output != '<center>' . __( 'No repositories defined.', 'installer' ) . '</center>' ) {
		...
		echo '<p>You can also use third party plugins and themes fully compatible with eCommerce Product Catalog. Please note that eCommerce Product Catalog developers get a small affiliate commision from every purchase made through the links below. This actually helps the devs to support the integration between plugins more effectively.</p>';
